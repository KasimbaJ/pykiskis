import { BookOpen } from 'lucide-react'
import type { Level } from '../../types'
import { useT } from '../../i18n-ui'

interface Props {
  level: Level
}

export default function LevelInstructions({ level }: Props) {
  const t = useT()
  const expectedOutputLabel: Record<Level['validationMode'], string> = {
    exact:    t('expectedOutput'),
    contains: t('level.outputMustContain'),
    regex:    t('level.outputMustMatch'),
  }
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2">
          <BookOpen className="w-5 h-5 text-blue-600" />
          <span className="text-xs font-semibold text-blue-600 uppercase">
            {t('level.id', { id: level.id })}
          </span>
        </div>
        {/* Concept badge */}
        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded-full">
          {level.concept}
        </span>
      </div>
      <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-2">{level.title}</h2>
      <p className="text-slate-600 dark:text-slate-300 mb-4 leading-relaxed">{level.description}</p>
      <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-100 dark:border-blue-800">
        <h3 className="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-1">{t('yourTask')}</h3>
        <p className="text-blue-700 dark:text-blue-300 text-sm">{level.task}</p>
      </div>
      <div className="mt-4 bg-slate-50 dark:bg-slate-700 rounded-lg p-4 border border-slate-200 dark:border-slate-600">
        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">
          {expectedOutputLabel[level.validationMode]}
        </h3>
        <pre className="text-sm text-slate-600 dark:text-slate-400 font-mono whitespace-pre-wrap">
          {level.expectedOutput}
        </pre>
      </div>
    </div>
  )
}
