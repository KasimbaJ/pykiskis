...
| `i18n-translate-ch1-plan.md` | **In progress — `arithmetic-operators` done** | Draft Lithuanian for Chapter 1 **theory** prose, one module per run, into draft PRs for teacher review. Only theory prose (already wired); never touches code/output/answers. `get-started`, `numbers-and-strings`, `comments`, `variables`, `output`, and `arithmetic-operators` translated. |
